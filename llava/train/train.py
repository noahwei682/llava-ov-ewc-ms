...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # EWC parameters
    use_ewc: bool = field(default=False, metadata={"help": "Whether to use Elastic Weight Consolidation (EWC) to prevent catastrophic forgetting."})
    ewc_lambda: float = field(default=0.0, metadata={"help": "Weight for EWC regularization term."})
...
def train(attn_implementation=None):
    ...
    # # Choose trainer based on EWC setting
    # if training_args.use_ewc:
    #     rank0_print(f"Using LLaVATrainerWithEWC (lambda={training_args.ewc_lambda})")
    #     trainer = LLaVATrainerWithEWC(
    #         model=model,
    #         tokenizer=tokenizer,
    #         args=training_args,
    #         **data_module
    #     )
    # else:
    #     rank0_print("Using standard LLaVATrainer")
    ...
    # Print EWC settings
    print("\n" + "!" * 80)
    print(f"{'EWC SETTINGS':^80}")
    print("!" * 80)
    print(f"Use EWC: {training_args.use_ewc}")
    print(f"EWC Lambda: {training_args.ewc_lambda}")
    print("!" * 80 + "\n")

    # Choose trainer based on EWC setting
    if training_args.use_ewc:
        print("\n" + "#" * 80)
        print(f"{'INITIALIZING EWC TRAINER':^80}")
        print("#" * 80 + "\n")
        trainer = LLaVATrainerWithEWC(
            model=model,
            tokenizer=tokenizer,
        ...
        print("\n" + "#" * 80)
        print(f"{'USING STANDARD TRAINER':^80}")
        print("#" * 80 + "\n")
        trainer = LLaVATrainer(
            model=model,
            tokenizer=tokenizer,
